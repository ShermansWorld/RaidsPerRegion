	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("source")) {
				...
				if (isConsole) {
					plugin.getLogger().info("Your server is running RaidsPerRegion Version 2.0 for Minecraft 1.18.2");
				} else {
					p.sendMessage("[RaidsPerRegion] Your server is running RaidsPerRegion Version 2.0 for Minecraft 1.18.2");
				}
